...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for input, gt in test_loader:
            ...
            # print(output)
            # pred_depth_image = torch.clamp(output, 1, 100)
            print(output)
            # print("hahaha", pred_depth_image)
            ...
            if is_data_aug:
                pred_depth_image = (max_depth/(pred_depth_image))/1000 #
                pred_depth_image[np.isinf(pred_depth_image)] = 100

            input_gt_depth_image /= np.max(input_gt_depth_image)
            pred_depth_image /= np.max(pred_depth_image)
            idx = idx + 1

            plot.imsave('./result/Test_input_rgb_{:05d}.png'.format(idx), input_rgb_image)
            plot.imsave('./result/Test_gt_depth_{:05d}.png'.format(idx), input_gt_depth_image, cmap="viridis")
            plot.imsave('./result/Test_pred_depth_{:05d}.png'.format(idx), pred_depth_image, cmap="viridis")
            ...
            log_pred = np.log(pred_depth_image+1e-10)
            log_gt = np.log(input_gt_depth_image+1e-10)
